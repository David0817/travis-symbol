...
describe('AccountQR -->', function () {
    describe('toJSON() should', function () {
        it('include mandatory NIP-7 QR Code base fields', function () {
            ...
            // Act:
            var exportAccount = new index_1.AccountQR(account, 'password', symbol_sdk_1.NetworkType.MIJIN_TEST, 'no-chain-id');
    ...
    describe('fromJSON() should', function () {
        ...
        it('reconstruct account given correct password', function () {
            ...
            // Assert
            chai_1.expect(importAccount.account.privateKey).to.be.equal(exportAccount.account.privateKey);
            chai_1.expect(importAccount.account.publicKey).to.be.equal(exportAccount.account.publicKey);
        });
        it('reconstruct account given correct ciphertext and password', function () {
            ...
            var importAccount = index_1.AccountQR.fromJSON(JSON.stringify(accountInfo), 'password');
            // Assert
            chai_1.expect(importAccount.account.privateKey).to.be.equal('749F1FF1972CD465CAB74566FF0AA021F846FBE3916ABB6A6C1373E962C76331');
            chai_1.expect(importAccount.account.publicKey).to.be.equal('50BFEB16AA0A3E00B8AB9385A2686991A09522DCDA4AC31B400BB5674EE4CDF8');
        });
    });
});
